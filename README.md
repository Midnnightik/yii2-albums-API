...
A small Yii2 REST JSON API for demo data (users, albums, photos) running locally with Docker.
...
## Quick start (Docker)
1) Configure the seed password (do NOT commit this file):
   ```bash
   cp config/seed_password_local.php.example config/seed_password_local.php
   # edit config/seed_password_local.php
   ```
2) Run the app:
   ```bash
...
## Local run details
For seeding-only and optional test execution, see `README_RUN.md`.
